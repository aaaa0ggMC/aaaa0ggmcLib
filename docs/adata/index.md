...
- [adata](#adata)
  ...
  - [总纲](#总纲-1)
  - [当前Features与规划](#当前features与规划-1)
    - [Features](#features-1)
    - [规划](#规划-1)
  - [性能](#性能-1)
    - [测试平台](#测试平台-1)
    - [VS nlohmann/json](#vs-nlohmannjson-1)
    - [校验性能](#校验性能-1)
      - [输出](#输出-2)
  - [CV              :2.431%](#cv--------------2431-1)
    - [加载json性能,主要得益于rapidjson SAX模式](#加载json性能主要得益于rapidjson-sax模式-1)
      - [输出](#输出-3)
  - [](#-1)
  - [CV              :1.422%](#cv--------------1422-1)
  - [详解](#详解-1)
    - [adata的基础架构](#adata的基础架构-1)
    - [AI总结出的解析](#ai总结出的解析)
...
```cpp
    // 1. 定义 Schema：支持类型约束、必填项、默认值填充
    AData data;
    data.load_from_memory(R"({
        "name" : "REQUIRED TYPE STRING VALIDATE not_empty",
        "id"   : ["TYPE INT", 1024, "<- 自动填充默认值"],
        "arr"  : ["TYPE ARRAY MIN 1", ["TYPE INT"]] 
    })");
    Validator vl = Validator::from_adata(data);
