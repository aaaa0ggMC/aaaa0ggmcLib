 ...
 * @brief Search algorithms implementations. / 搜索算法
 * @version 5.0
 * @date 2026-04-13
 * 
...
namespace alib5::algo::search {
    ...
    template<class T> 
    concept IsIterator = std::random_access_iterator<T> || std::is_pointer_v<T>;
    ...
    T1 plain_pattern(T1 data_begin, T1 data_end, size_t data_size, T2 pattern_begin, T2 pattern_end, size_t pattern_size);

    /**
     * @brief Simplified override for naive pattern matching algorithm (auto-infers length).
     */
...
namespace alib5::algo::search {
    ...
    template<IsIterator T1, IsIterator T2, bool safe_mode>
    requires requires(T1& a, T2& b) { *a != *b; }
    T1 plain_pattern(T1 data_begin, T1 data_end, size_t data_size, T2 pattern_begin, T2 pattern_end, size_t pattern_size) {
    ...
    requires requires(T1& a, T2& b) { *a != *b; }
    T1 plain_pattern(T1 data_begin, T1 data_end, T2 pattern_begin, T2 pattern_end) {
        return plain_pattern<T1, T2, false>(
            data_begin, data_end, std::distance(data_begin, data_end),
            pattern_begin, pattern_end, std::distance(pattern_begin, pattern_end)
        );
    }
...
#endif // ALIB5_ALGO_SEARCH_H_INCLUDED