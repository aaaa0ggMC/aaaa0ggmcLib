 ...
 * @brief Schema generation support.
 * 
...
namespace alib5::detail {
    ...
    /**
     * @brief Generates an AData schema representation for a given type.
     * 
...
namespace alib5::detail {
    ...
    inline AData _generate_schema() {
        ...
        if constexpr(std::is_enum_v<std::decay_t<T>>) {
            ...
            root[0] = "TYPE DOUBLE";
        } else if constexpr(IsStringLike<std::decay_t<T>> || IsStringLike<T>) {
            root[0] = "TYPE STRING";
        } else if constexpr(
            requires {
        ...
        } else if constexpr(std::meta::is_class_type(^^decayed_T)) {
            add_constaint = false;
            ...
            template for(
                ...
                if constexpr(!std::meta::is_reference_type(element_type_info)) {
                    ...
                    // Directly skip if needed. (Original: 对于需要skip的直接进行skip)
                    if constexpr(
        ...
        if(add_constaint) {
            ...
            // Here we check the constraint. (Original: 这里是检测constraint)
            template for(constexpr auto anno : annotations) {
                ...
                if constexpr(requires{ AnnotationBaseType::attribute_trait; }) {
