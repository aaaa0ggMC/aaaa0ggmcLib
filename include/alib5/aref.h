/**
 * @file aref.h
 * @brief Non-dangling indexed references into containers with single and multi-level access. / 不悬垂的容器索引引用，支持单层与多层访问。
 * @author aaaa0ggmc
 * @date 2026/06/18
 * @version 5.0
 ...
 * 不会悬垂的比较安全的容器数据wrapper,Release下单次性能损失为0.3ns
 *
...
namespace alib5{
    /// @brief Concept requiring container @p T expose operator[], size(), empty(), value_type, and reference.
    ...
    /// @brief Concept constraining to integer-like index types accepted by RefWrapper indexing.
    ...
    /**
     * @brief Index-based wrapper referencing one slot of a container; survives container moves, debug-bounded.
     *
     * @par Original Comment:
     * 通过持有index从而保证一定的安全性，Release优化后仅仅多出0.3ns(5.7Ghz电脑)的解引用时长
     *
     * @tparam Cont
     */
    template<CanAccessItem Cont> struct RefWrapper{
        using BaseType = typename Cont::value_type;
        /// @brief Pointer to the owning container; caller must keep it alive. 容器的引用，所以你需要保证至少容器是存在的，嵌套容器就可能不太支持了
        ///
        /// @par Original Comment:
        /// 容器的引用，所以你需要保证至少容器是存在的，嵌套容器就可能不太支持了
        /// 不行了，受不了了，准备改成指针
        Cont * cont { nullptr };
        /// @brief Current index into the container; max size_t means unset. index
        std::size_t index { std::numeric_limits<size_t>::max() };

        /// @brief True when the current index still points to a live element. 确认现在是否还存在数据
        inline bool has_data(){
            return index < cont->size();
        }

        /// @brief True when cont is non-null and the index is in range. 综合确认是否有效
        inline bool valid(){
            if(cont){
                return has_data();
            }
        ...
        /// @brief Return a reference to the element, with debug bounds checks. 获取当前的引用
        inline auto& get() {
            // debug下防止shrink
        ...
        /// @brief Return a pointer to the element, with debug bounds checks.
        inline auto* ptr() {
            // debug下防止shrink
            panic_debug(!cont,"Root container is empty!");
            panic_debug(index >= cont->size(),"Index out of bounds!");
        ...
        /// @brief Pointer access to the referenced element. 访问应用内部
        inline auto* operator->(){
            return &(get());
        }

        /// @brief Generate forwarding operators for arithmetic, assignment, and comparison.
        ///
        /// @par Original Comment:
        /// 尝试调用引用的赋值
        ...
        /// @brief Pre-increment the referenced value.
        inline auto operator++()    requires requires { ++get(); } { return ++get(); }
        /// @brief Post-increment the referenced value.
        inline auto operator++(int) requires requires { get()++; } { return get()++; }
        /// @brief Pre-decrement the referenced value.
        inline auto operator--()    requires requires { --get(); } { return --get(); }
        /// @brief Post-decrement the referenced value.
        inline auto operator--(int) requires requires { get()--; } { return get()--; }

        /// @brief Advance the index by one and return *this.
        inline RefWrapper& next_ref() {
            return set_index(index + 1);
        }

        /// @brief Step the index back by one and return *this.
        inline RefWrapper& prev_ref() {
            return set_index(index - 1);
        }

        /// @brief Shift the index by @p offset and return *this.
        inline RefWrapper& advance_ref(int offset) {
            return set_index(index + offset);
        }

        /// @brief Return a new wrapper pointing @p offset slots away from this one.
        ...
        /// @brief Set the index with debug bounds checks; returns *this for chaining.
    ...
    /// @brief Build a single-level RefWrapper for @p cont at @p index, with debug bounds check.
    ...
    /**
     * @brief Multi-layer reference holding a pointer to the root container and one index per level.
     *
     * @par Original Comment:
     * 多层引用容器。
    ...
    template<CanAccessItem Cont,size_t N> struct MultiRefWrapper{
        ...
        /// @brief Pointer to the root container; caller must keep it alive. 底层容器，需要保证不悬垂
        Cont * cont;
        /// @brief Per-level indices into nested containers. 存储索引的数组
        std::array<size_t,N> indices;
    private:

        /// @brief Recursively walk indices, returning a reference to the innermost element. 递归获取对象，每次都会进行一次检查
        ...
        /// @brief Base case: assign the final index. 最后一层调用
        ...
        /// @brief Recursive step: assign each index with bounds checks against the matching level. 递归设置索引 每次都会进行一次检查
    ...
    public:
        /// @brief Down-cast a deeper MultiRefWrapper, copying its leading indices.
        ...
        /// @brief Up-cast by appending one more index to a shallower MultiRefWrapper.
        ...
        /// @brief Construct a multi-level reference over @p c at the given indices. 构造引用对象
        template<IsNumber... Args> inline MultiRefWrapper(Cont & c,Args&&... val):cont(&c){
            set_indices(std::forward<Args>(val)...);
        }

        /// @brief Return a reference to the innermost addressed element. 获取内存对象
        inline auto& get(){
            panic_debug(!cont, "Root container is empty!");
            return get_raw<Cont,0>(*cont);
        }

        /// @brief Re-assign all indices, with per-level bounds checks. 设置索引
        ...
        /// @brief Pointer access to the innermost element. 获取当前对象
        inline auto* operator->(){
            return &get();
        }

        /// @brief Return a shallower MultiRefWrapper addressing one fewer level. 往上面走一层
        inline auto operator()(){
            static_assert(N != 1,"You have reached to the root of the container!");
            return MultiRefWrapper<Cont,N-1>(*this);
        }

        /// @brief Return a copy with the innermost index replaced by @p index. 设置当前层索引
        ...
        /// @brief Return a deeper MultiRefWrapper appending @p index as a new innermost level. 往下走一层
        inline auto operator[](size_t index){
        ...
        /// @brief Assign to the innermost element.
        ///
        /// @par Original Comment:
        /// 尝试调用引用的赋值
    ...
    /// @brief Build a MultiRefWrapper over @p cont with one index per nested level. 获取多层container的引用
